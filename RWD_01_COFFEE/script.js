  ...
  document.querySelector("#roller2").style.left = document.querySelector(".award-item-inner div").offsetWidth + "px";
  ...
  $(document).ready(function () {
    ...
    $(".tablet-drawer-menu").click(function () {
      ...
      if (checkClass == true) {
        $(".tablet-gnb-menu-list").slideDown(); //서브메뉴 내려옴
        $(".back-bg ").addClass("active"); // 뒷배경 어둡게
        /* 스크롤 방지 */
        $("html, body").css({
          overflow: "hidden",
    ...
    $(window).scroll(function () {
      $(".brand-item").each(function () {
        ...
        let windowTop = $(window).scrollTop(); //현재스크롤 top 위치
    ...
    let locations = {
      ...
      2: ["계양구", "남구", "남동구", "동구", "부평구", "서구", "연수구", "중구", "강화군", "옹진군", ],
      3: ["대덕구", "동구", "서구", "유성구", "중구"],
      4: ["광산구", "남구", "동구", "북구", "서구"],
      5: ["남구", "달서구", "동구", "북구", "서구", "수성구", "중구", "달성군"],
      6: ["남구", "동구", "북구", "중구", "울주군"],
      7: ["강서구", "금정구", "남구", "동구", "동래구", "부산진구", "북구", "사상구", "사하구", "서구", "수영구", "연제구", "영도구", "중구", "해운대구", "기장군", ],
      8: ["고양시", "과천시", "광명시", "광주시", "구리시", "군포시", "김포시", "남양주시", "동두천시", "부천시", "성남시", "수원시", "시흥시", "안산시", "안성시", "안양시", "양주시", "오산시", "용인시", "의왕시", "의정부시", "이천시", "파주시", "평택시", "포천시", "하남시", "화성시", "가평군", "양평군", "여주군", "연천군"],
      9: ["강릉시", "동해시", "삼척시", "속초시", "원주시", "춘천시", "태백시", "고성군", "양구군", "양양군", "영월군", "인제군", "정선군", "철원군", "평창군", "홍천군", "화천군", "횡성군", ],
      10: ["제천시", "청주시", "충주시", "괴산군", "단양군", "보은군", "영동군", "옥천군", "음성군", "증평군", "진천군", "청원군"],
      11: ["계룡시", "공주시", "논산시", "보령시", "서산시", "아산시", "천안시", "금산군", "당진군", "부여군", "서천군", "연기군", "예산군", "청양군", "태안군", "홍성군", ],
      12: ["군산시", "김제시", "남원시", "익산시", "전주시", "정읍시", "고창군", "무주군", "부안군", "순창군", "완주군", "임실군", "장수군", "진안군"],
      13: ["광양시", "나주시", "목포시", "순천시", "여수시", "강진군", "고흥군", "곡성군", "구례군", "담양군", "무안군", "보성군", "신안군", "영광군", "영암군", "완도군", "장성군", "장흥군", "진도군", "함평군", "해남군", "화순군", ],
      14: ["경산시", "경주시", "구미시", "김천시", "문경시", "상주시", "안동시", "영주시", "영천시", "포항시", "고령군", "군위군", "봉화군", "성주군", "영덕군", "영양군", "예천군", "울릉군", "울진군", "의성군", "청도군", "청송군", "칠곡군", ],
      15: ["거제시", "김해시", "마산시", "밀양시", "진주시", "진해시", "창원시", "통영시", "거창군", "고성군", "남해군", "산청군", "의령군", "창녕군", "하동군", "함안군", "함양군", "합천군"],
    ...
    function selectLocation() {
      ...
      for (var key in locations) { //for ~ in 을 사용해서 locations객체의 키 값을 하나씩 가져옴
        if (key == selectSido) {
          let locationsLength = locations[selectSido].length; //key값이 선택한 시/도와 같으면 다음 실행
          for (let i = 0; i < locationsLength; i++) {
            $("#gugun").append("<option value='" + i + "'>" + locations[selectSido][i] + "</option>");
          }
        }
      }
    }
    $("#sido").on("change", selectLocation); //시도 select option 변경시 selectLocation 함수실행

    // text양식, 개인정보동의 체크 됐는지 확인
    function formCheck(text) {
      let agreeChecking = $("#agreeCheck").is(":checked");
      let nameChecking = $("#name").val();
      let phoneChecking = $("#phone").val();
      let eMailChecking = $("#e-mail").val();
      if (nameChecking == "") {
        alert("성함을 입력해주세요");
        ...
        alert("이메일을 입력해주세요");
      } else if (agreeChecking == false) {
        alert("개인정보수집이용동의를 체크해주세요");
      } else {
        alert(text + " 신청이 완료되었습니다.");
      }
    }
    //상담신청 버튼 클릭시 formCheck 실행
    $("#applyConsulting").click(function () {
      formCheck("상담");
    });
    //창업설명회 버튼 클릭시 formCheck 실행
    ...
    // 베스트메뉴 슬라이드-모바일버전 
    let widthSizing = $(window).width();
    $(".best-menu-mobile").css("width", widthSizing - 40);
    $(".best-menu-mobile img").css("width", widthSizing - 60);
    let i = 0;
    ...
    function slideLeft() {
      ...
      $(".best-menu-mobile .best-menu-item-inner").animate({
